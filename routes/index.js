...
router.get('/clearCart', async (req, res, next) => {
  const userId = "662006abcc67de88ff29ea2f";
...
router.post('/updateBalance', checkAuth, async (req, res, next) => {
    const userId = req.userId;
    console.log(userId)
...
router.post('/viewCart', checkAuth, async (req, res, next) => {
	const userId = req.userId;
  try {
...
router.get('/add', async (req, res, next) => {
  const userId = '662006abcc67de88ff29ea2f';
  const tagToAdd = 'e280699500004009f12729a1';
  ...
  try {
    ...
    console.log(`Tag '${tagToAdd}' added to user with ID '${userId}'`);

    res.status(200).json({ success: true, msg: `Tag '${tagToAdd}' added to user` });
  } catch (error) {
